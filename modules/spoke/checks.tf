...
check "stackset" {
  # You must define a region when using stacksets
  assert {
    condition     = !(var.enable_stackset == true && var.region != null)
    error_message = "When 'enable_organizations' is set to true, 'region' must be set"
  }
}
