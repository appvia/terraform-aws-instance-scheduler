...
- Terraform references the shared scheduler template bucket in the hub account.
- The remote CloudFormation template is consumed from the published scheduler path.
- The module deploys a standalone stack for a single spoke account.
- Optional macro integration enables default-tag injection on spoke-side CloudFormation resources.

## Capabilities

- **Security by default**: Consumes remote templates from the hardened scheduler bucket with transport and encryption policy protections.
